...
const BurgerKing= () => {
  ...
  useEffect(() => {
    fetchBurgerKingProducts();
  }, []);

  const fetchBurgerKingProducts = async () => {
    setLoading(true);
    try {
      const response = await axios.get(
        "http://localhost:5000/user/products/category/BurgerKing",
  ...
  return (
    <div>
      <Bkcarousel /> {/* Move Scoopscarousel to the top */}
