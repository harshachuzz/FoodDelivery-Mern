...
const Dominospizza= () => {
  ...
  useEffect(() => {
    fetchdominospizzaProducts();
  }, []);

  const fetchdominospizzaProducts = async () => {
    setLoading(true);
    try {
      const response = await axios.get(
        "http://localhost:5000/user/products/category/dominos",
  ...
  return (
    <div>
      <Dominospizzacarousel /> {/* Move Scoopscarousel to the top */}
